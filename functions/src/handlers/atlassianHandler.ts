...
async function updateConfluencePage(pageId: string, title: string, contentMarkdown: string) {
    ...
    if (!getRes.ok) throw new Error(`Failed to fetch page ${pageId}: ${await getRes.text()}`);
    const currentData = await getRes.json();
    body.version.number = (currentData.version?.number || 1) + 1;
...
async function createJiraIssue(summary: string, description: string, issueType: string = 'Task') {
    if (!DOMAIN) throw new Error("Missing ATLASSIAN_DOMAIN");
...
export async function handleAtlassianSync(db: Firestore, jobId: string, jobData: JobData) {
